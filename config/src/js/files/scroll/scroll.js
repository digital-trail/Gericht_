...
// ? Проверяет количество прокрученых пикселей(по умолчанию это 1 пикс) и будет добавлять класс '_header-scroll' к шапке
// Работа с шапкой при скроле
export function headerScroll() {
  addWindowScrollEvent = true;
  const header = document.querySelector("header.header");
  //   ? "data-scroll-show" это data атрибут который включает функционал позволяющий шапке исчезать, при скроле вниз, а при скроле вверх оставаться все время на месте
      ...
      stickyParents.forEach((stickyParent) => {
        let stickyConfig = {
          top: stickyParent.dataset.stickyTop
            ? stickyParent.dataset.stickyTop
            : 0,
          bottom: stickyParent.dataset.stickyBottom
            ? stickyParent.dataset.stickyBottom
            : 0,
